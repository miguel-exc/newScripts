#!/usr/bin/env python3
"""
f1_all.py — F1 Line-Rate Testing (Unificado) — VERSIÓN HOMOGENIZADA
...
Fat-Tree: usa múltiples caminos de forma natural por diseño.

Ejecutar desde H9 (Mac M1):
    python3 f1_all.py --experiment all --topology sl
    python3 f1_all.py --experiment all --topology ft
"""
...
EXPERIMENTS = {
    ...
    "b2": {
        "desc": "4 pares cross-leaf CON balanceo manual (usando S1 y S2)",
        "note": "F1-B2: 4 flujos full-mesh, con balanceo manual entre S1 y S2",
        "pairs": [
...
def kill_iperf_all(pairs: List[dict]) -> None:
    ...
    Necesario porque un timeout de SSH local no garantiza que el
    proceso iperf3 remoto (lado cliente) muera con la sesión, lo que
    puede dejar clientes zombie contaminando repeticiones siguientes."""
...
def start_servers(pairs: List[dict]) -> None:
    kill_iperf_all(pairs)   # limpia clientes Y servidores antes de arrancar
    time.sleep(1)
    for p in pairs:
...
def run_single_pair(pair: dict, pkt_size: int, rep: int, topology: str,
    ...
    try:
        ...
        results[pair["id"]] = None
        # Si el SSH falló/timeouteó, el iperf3 remoto puede seguir vivo
        # (sin pty, el SIGHUP no siempre se propaga). Lo matamos ya
        # mismo para no contaminar la siguiente repetición.
...
def run_experiment(experiment: str, topology: str, dry_run: bool) -> None:
    ...
    for pkt_size in PKT_SIZES:
        ...
        if not dry_run:
            kill_iperf_all(pairs)   # limpia clientes Y servidores al cerrar este pkt_size
...
def preflight(experiment: str) -> bool:
...
def main():
    parser = argparse.ArgumentParser(
        description="F1: Line-Rate Testing Unificado (HOMOGENIZADO)"
    )
    parser.add_argument(
        "--experiment",
