#!/usr/bin/env python3
"""
f3_all.py — F3 Jitter Testing (Unificado) — VERSIÓN HOMOGENIZADA
RFC 1889 / RFC 8239 — Testbed SDN Spine-Leaf vs Jerárquica 3 Capas — UASLP 2026

CORRECCIONES METODOLÓGICAS (2026-07-03):
  - Cooldown estandarizado: 5s (ya lo era, ahora se registra en _meta)
  - _meta completo: cooldown_s, snd_cwnd (N/A para UDP), udp_payload_bytes
  - Summary: se añade cooldown_s, jitter_p95_ms, jitter_std_ms
  - FIX: H4->H5 y H7->H8 eran intra-leaf (no cruzaban ningún spine) y se
    reemplazaron por H4->H8 y H7->H2, cubriendo las 3 combinaciones de
    leaf-pair (L1-L2, L2-L3, L3-L1). No se etiqueta S1/S2 explícitamente
    porque el mapeo físico puerto-uplink->spine aún no está confirmado.
...
# ══════════════════════════════════════════════════════════════════════════════
#  CONFIGURACIÓN POR TOPOLOGÍA (CON BALANCEO MANUAL PARA SL)
# ══════════════════════════════════════════════════════════════════════════════
...
TOPOLOGIES = {
    "sl": {
        ...
        # CORREGIDO: los 3 pares ahora cruzan un leaf distinto de verdad
        # (antes H4->H5 y H7->H8 eran intra-leaf, no atravesaban ningún spine).
        # Cubre las 3 combinaciones de leaf-pair posibles: L1-L2, L2-L3, L3-L1.
        # NOTA: no se etiqueta como "S1" o "S2" porque el mapeo físico
        # puerto-uplink -> spine (UPLINK_PORT) todavía no está confirmado.
        "emisores": ["H1", "H4", "H7"],
        "destino": {"H1": "H5", "H4": "H8", "H7": "H2"},  # Destino por emisor
        "puertos": {"H1": 5301, "H4": 5302, "H7": 5303},
        "scenario_desc": "Cruce de spine en las 3 combinaciones de leaf: H1(L1)->H5(L2), H4(L2)->H8(L3), H7(L3)->H2(L1)",
    },
    "j3c": {
        "hosts": {
            "H1": {"ip": "10.0.1.1", "user": "h1"},
        ...
        "scenario_desc": "H1+H2+H3 (Edge1) → H8 (Edge2) — cruzando Core1",
    }
}
    ...
    while rep < REPS_MAX:
        ...
        record = {
            "_meta": {
                ...
                "emisores": emisores,
                "destino_por_emisor": SCENARIO["destino"],  # NUEVO: documenta balanceo
            },
...
def run_experiment(topology, rates, dry_run):
    ...
    if topology == "sl":
        print(f"  Cruce de spine: H1(L1)->H5(L2), H4(L2)->H8(L3), H7(L3)->H2(L1)")
